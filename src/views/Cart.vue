<template>
  <div class="cart">
    ...
    <div id="deleteCart" v-if="$store.state.cart != 0" style="text-align: right">
      <v-btn color="error" @click="deleteCart">
        Apagar carrinho
      </v-btn>
    </div>
    ...
    <v-card class="mx-auto" max-width="944" outlined>
      <v-list-item three-line v-for="car in $store.state.cart" :key="car.id">
        <input class="form-check-input" type="checkbox" name="checkbox" @click="chekboxLength" />
        <img :src="car.url" style=" margin-right: 10px;
          width: 150px; margin-bottom: 80px;" />
        ...
        <v-list-item-content>
          ...
          <v-list-item-subtitle class="pt-4"> {{ car.category }}</v-list-item-subtitle>
          <strong>
            <v-list-item-subtitle class="pt-4"> {{ car.price.toLocaleString('pt-br',{style: 'currency',
          ...
          <div class="card-footer">
            ...
            <small>Frete grátis em fretes até R$40,00 para pedidos acima de
              R$79,00 <a href="#" class="saibaMais">Saiba mais</a></small>
          </div>
